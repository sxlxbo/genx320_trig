...
LED_STABILIZE_DELAY = 0.005   # LED 稳定时间：5ms（确保颜色切换完成）
TRIGGER_PULSE_WIDTH = 0.001    # Trigger 脉冲宽度：1ms（确保相机能检测到）
...
def sync_led_and_trigger():
    ...
    try:
        while time.time() - start_time < TOTAL_DURATION:
            ...
            # 3. 【等灯稳定】：跳过颜色切换的过渡时间（关键！）
            time.sleep(LED_STABILIZE_DELAY)
            
            # 4. 【后打触发】：此时 LED 已经是稳定的新颜色了
            trigger_pin.on()
            time.sleep(TRIGGER_PULSE_WIDTH)  # 保持脉冲宽度
            trigger_pin.off()
